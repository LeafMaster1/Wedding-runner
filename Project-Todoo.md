...
- [ ] **Ladda karaktärer**: Uppdatera `src/game/scenes/Preloader.ts` för att ladda `wedding-cloth-run.png` (som en spritesheet om det är en animation).
- [ ] **Skapa animationer**: I `Preloader.ts` (create) eller `Game.ts`, definiera animationer för:
    - `run`: Spelaren springer framåt.
    - `jump`: Spelaren hoppar.

## 3. Spelarlogik (Game.ts)
- [ ] **Skapa spelaren**: Lägg till `this.player = this.physics.add.sprite(...)` i `create()`-metoden.
...
## 4. Styrning & Input
- [ ] **Tangentbordsstyrning**: Lägg till lyssnare för `SPACE` eller `UP` för att hoppa, samt piltangenter för framåt/bakåt.
- [ ] **Touch-kontroller (Mobil)**:
    - [ ] **Vänsterpil**: Skapa en knapp för att gå bakåt.
    - [ ] **Högerpil**: Skapa en knapp för att gå framåt.
    - [ ] **Upp-pil (Hopp)**: Skapa en dedikerad knapp för att hoppa.
...
- [ ] **Placering**: Placera vänster- och högerpilar nere i vänstra hörnet och hoppknappen nere i högra hörnet för bästa ergonomi.
...
## 6. Finslipning
- [ ] **Partikeleffekter**: Lägg till lite "damm" när man springer eller landar.
- [ ] **Ljud**: Spela upp ett hopp-ljud när man trycker på knappen.
